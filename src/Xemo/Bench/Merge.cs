...
public static class Merge
{
    ...
    {
        return new Merge<TTarget>(target, solve1to1, solve1toMany);
    }
    ...
    {
        return new Merge<TTarget>(target);
    }
}
...
) : IBench<TResult>
{
    ...
    {
        return First._(type.GetConstructors())
            .Value()
            .Invoke(values);
    }
    ...
    {
        var result = Instance(type);
        for (var i = 0; i < propInfos.Length; i++) propInfos[i].SetValue(result, values[i]);
        return result;
    }
    ...
    private static object Instance(Type type)
    {
        ...
        if (ctor == null)
            throw new ArgumentException("A DTO must have a parameterless constructor to use it in a cocoon.");
        return ctor.Invoke([]);
    }
    ...
    private static bool IsCompatible(PropertyInfo targetProp, PropertyInfo sourceProp)
    {
        ...
        if (IsLive(sourceProp)) source = sourceProp.PropertyType.GenericTypeArguments[0];
    ...
    {
        return
            (input.IsArray ? Type.GetTypeCode(input.GetElementType()) : Type.GetTypeCode(input)
            ) != TypeCode.Object;
    }
